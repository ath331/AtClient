...
bool Handle_S_LOGIN(PacketSessionPtr& session, Protocol::S_LOGIN& pkt);
bool Handle_S_ENTER_GAME(PacketSessionPtr& session, Protocol::S_ENTER_GAME& pkt);
...
public:
	static void Init()
	{
		...
		GPacketHandler[ PKT_S_LOGIN ] = []( PacketSessionPtr& session, BYTE* buffer, int32 len ) { return HandlePacket<Protocol::S_LOGIN>( Handle_S_LOGIN, session, buffer, len ); };
		GPacketHandler[ PKT_S_ENTER_GAME ] = []( PacketSessionPtr& session, BYTE* buffer, int32 len ) { return HandlePacket<Protocol::S_ENTER_GAME>( Handle_S_ENTER_GAME, session, buffer, len ); };
	...
	static SendBufferPtr MakeSendBuffer( Protocol::C_ENTER_GAME& pkt ) { return MakeSendBuffer( pkt, PKT_C_ENTER_GAME ); }
